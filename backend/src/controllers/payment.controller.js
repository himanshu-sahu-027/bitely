...
import {
  createDummyPaymentVerification,
  createDummyRazorpayOrder,
} from "../utils/dummyPaymentGateway.js";
...
const createGatewayOrder = async ({ amount, receipt }) => {
  // Dummy mode keeps frontend/backend development separate until you want
  // to switch to the real Razorpay test order creation flow.
  if (config.USE_DUMMY_PAYMENT || !isRazorpayConfigured()) {
    return createDummyRazorpayOrder({ amount, receipt });
  }
  ...
  try {
    ...
    return sendResponse(res, {
      ...
      data: {
        order,
        razorpayOrder,
        keyId: config.RAZORPAY_KEY_ID || "rzp_test_dummy_key",
        mode: config.USE_DUMMY_PAYMENT ? "DUMMY_ONLINE" : "RAZORPAY_TEST",
      },
    });
  } catch (error) {
    next(error);
...
export const verifyPayment = async (req, res, next) => {
  try {
    ...
    const verificationSecret =
      config.USE_DUMMY_PAYMENT || !isRazorpayConfigured()
        ? createDummyPaymentVerification({ razorpayOrderId }).secret
